...
static bool isPointInRect(ofxVec2f p, ofRectangle r){
...
void THISKeyframeEditor::draw()
{
	...
	for(int i = 0; i < keyframes.size(); i++){
		...
		ofxVec2f screenpoint = coordForKeyframePoint(keyframes[i]->position);
...
void THISKeyframeEditor::loadFromXML()
{
	...
	for(int i = 0; i < numKeyTags; i++){
		...
		THISKeyframe* key = newKeyframe(ofxVec2f(savedkeyframes.getValue("x", 0.0),
...
void THISKeyframeEditor::reset()
{
	...
	firstkey = newKeyframe( ofxVec2f(0, .5) );
	lastkey = newKeyframe( ofxVec2f(1.0, .5) );
}
...
void THISKeyframeEditor::mousePressed(ofMouseEventArgs& args)
{
	...
	ofxVec2f screenpoint = ofxVec2f(args.x, args.y);
	if(drawingEasingWindow){
...
void THISKeyframeEditor::mouseDragged(ofMouseEventArgs& args)
{
	...
	if(hasFocus && selectedKeyframe != NULL){
		ofxVec2f newposition = keyframePointForCoord(ofxVec2f(args.x, args.y) - grabOffset);
...
THISKeyframe* THISKeyframeEditor::keyframeAtScreenpoint(ofxVec2f p, int& selectedIndex){
	float mindistance = 15;
	for(int i = 0; i < keyframes.size(); i++){
		ofxVec2f keyonscreen = coordForKeyframePoint(keyframes[i]->position);
...
ofxVec2f THISKeyframeEditor::coordForKeyframePoint(ofxVec2f keyframePoint)
{
	return ofxVec2f(ofMap(keyframePoint.x, zoomBounds.min, zoomBounds.max, bounds.x, bounds.x+bounds.width, true),
				   ofMap(keyframePoint.y, 1.0, 0.0, bounds.y, bounds.y+bounds.height, true));
}

bool THISKeyframeEditor::screenpointIsInBounds(ofxVec2f screenpoint)
{
	return isPointInRect(screenpoint, bounds);
}

ofxVec2f THISKeyframeEditor::keyframePointForCoord(ofxVec2f coord)
{
	return ofxVec2f(ofMap(coord.x, bounds.x, bounds.x+bounds.width,  zoomBounds.min, zoomBounds.max, true),
				   ofMap(coord.y, bounds.y, bounds.y+bounds.height, 1.0, 0.0, true));
}

THISKeyframe* THISKeyframeEditor::newKeyframe(ofxVec2f point)
{
